...
//		System.out.println(sCfgUtil  + ": "  + boolToString(cfgUtil));
	...
	// ==== private Methods
	private static void showGlobalTestResult() {
		showLocalTestResult ();
		if (verbose >= GLOBAL_INFO
				|| (verbose >= GLOBAL_FAIL && !globalPass)) {
			System.out.println("Test status: "
				+ globalTest + ": " + boolToString(localPass)
				+ " -> overall = " + boolToString(globalPass));
		}
	}
	private static void showLocalTestResult () {
		if (verbose >= LOCAL_INFO
				|| (verbose >= LOCAL_FAIL && !localPass)) {
			System.out.println("status of \""+ localTest + "\" = " + boolToString(localPass) );
		}
	}
	...
	private static void showGlobalTestStart (String name) {
		globalTest = name;
		localTest = "";
		localPass = true;
		if (verbose >= GLOBAL_INFO) {
			System.out.println("Start Test \""+ globalTest + "\" :" );
		}
	}
	private static void showTestError (Exception e) {
		showTestError (e.toString());
	}
	private static void showTestError (String err) {
		localPass  = false;
		globalPass = false;
		if (verbose >= LOCAL_FAIL) {
			System.out.println("Failed Test: " + globalTest + " at " + localTest);
			System.out.println("Error = " + err);
		}
	}
	private static void newTest(String test) {
		if(!localTest.isBlank() && verbose >= LOCAL_INFO) {
			showLocalTestResult ();
		}
		localTest = test;
		if (verbose >= LOCAL_INFO) {
			System.out.println(" - Test: " + localTest);
		}
	}
